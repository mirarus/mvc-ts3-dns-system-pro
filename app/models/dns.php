...
class dns extends model
{
	...
	public function UpdateDNS($mail, $id, $dns, $name, $domain, $ip, $port)
	{
		$User_Data = controller::model('user')->GetUserByMail($mail);
		if ($User_Data) {
			return $this->query('UPDATE dns SET dns=?, name=?, domain=?, ip=?, port=?, lastupdate_time=? WHERE id=? AND u_id=?', [$dns, $name, $domain, $ip, $port, time(), $id, $User_Data['id']]);
		}
	}
	...
	public function UpdateDNSAdmin($username, $id, $dns, $name, $domain, $ip, $port)
	{
		$Admin_Data = controller::model('admin')->GetAdminByUsername($username);
		if ($Admin_Data) {
			return $this->query('UPDATE dns SET dns=?, name=?, domain=?, ip=?, port=? WHERE id=?', [$dns, $name, $domain, $ip, $port, $id]);
		}
	}
