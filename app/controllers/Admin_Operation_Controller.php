...
class Admin_Operation_Controller extends controller
{
	...
	public function DNS_Edit_Action()
	{
		$this->LoginAccess();
		$_REQUEST = FilterClass::filterXSS($_REQUEST);
		if ($_SERVER['REQUEST_METHOD'] === 'GET') {
			...
			$id		= Gauntlet::filter($_REQUEST['id']);
			$name	= Gauntlet::filter(strtolower($_REQUEST['name']));
			$ip		= Gauntlet::filter($_REQUEST['ip']);
			$port	= Gauntlet::filter($_REQUEST['port']);
			if (isset($id) && isset($name) && isset($ip) && isset($port) && !empty($id) && !empty($name) && !empty($ip) && !empty($port)) {
				if (preg_match("/^[a-zA-Z0-9]*$/",$name)) {
					if ($name != "ftp" || $name != "www" || $name != "A" || $name != "a" || $name != "w" || $name != "W" || $name != "ts3" || $name != "TS3") {
						if (valid_ip_adress($ip) == true) {
							...
							$DNS_	= $this->model('dns')->GetDNSAdminX($_SESSION['admin_username'], $id);
							$Mirarus_Dns	= new mirarus_dns($dns_config['mail'],$dns_config['apikey']);
							$Mirarus_Dns_identifier	= $Mirarus_Dns->identifier($DNS_['domain']);
							$Mirarus_Dns_A_Record_ID_F		= $Mirarus_Dns->get_dns_record_id_a($Mirarus_Dns_identifier, $DNS_['dns']);
							$Mirarus_Dns_SRV_Record_ID_F	= $Mirarus_Dns->get_dns_record_id_srv($Mirarus_Dns_identifier, '_ts3._udp.'.$DNS_['dns']);
							if ($DNS_ && $Mirarus_Dns_A_Record_ID_F && $Mirarus_Dns_SRV_Record_ID_F) {
								$dns	= (strtolower($name).'.'.$DNS_['domain']);
								$UpdateDBDNS = $this->model('dns')->UpdateDNSAdmin($_SESSION['admin_username'], $id, $dns, $name, $DNS_['domain'], $ip, $port);
								$Mirarus_Dns_A_Record_UPDATE	= $Mirarus_Dns->update_a_record($Mirarus_Dns_identifier,$Mirarus_Dns_A_Record_ID_F,$name,$ip);
								$Mirarus_Dns_SRV_Record_UPDATE	= $Mirarus_Dns->update_srv_record($Mirarus_Dns_identifier,$Mirarus_Dns_SRV_Record_ID_F,$name,$port,$DNS_['domain']);
								if ($UpdateDBDNS && $Mirarus_Dns_A_Record_UPDATE && $Mirarus_Dns_SRV_Record_UPDATE) {
									echo alert('', 'DNS Başarıyla Düzenlendi, Lütfen Bekleyiniz!', 'success', 0, "mdi mdi-check-all");
									refresh(SITE_URL.'/admin/dns', 3);
								} else{
									echo alert('Hata!', 'DNS Düzenlenirken Bir Hata Oluştu!', 'warning', 0, "mdi mdi-alert");
								}
							} else{
								echo alert('Hata!', 'Bu Bilgilere Sahip Bir DNS Kaydı Bulunmuyor!', 'warning', 0, "mdi mdi-alert");
							}
						} else{
