					...
					<table class="table">
						...
						<tbody>
							<?php foreach ($DNS_Data_F as $DNS_Data_R) { ?>
								<?php $User_Data_S = controller::model('user')->GetUserByID($DNS_Data_R['u_id']); ?>
								<tr>
									...
									<td>
										<a href="#" onclick="window.location.href='<?php echo SITE_URL; ?>/admin/dns/<?php echo $DNS_Data_R['id']; ?>'">
											<label class="badge badge-gradient-info">Düzenle</label>
										</a>
									</td>
									<td>
										<a href="#" onclick="DNSDelete('<?php echo SITE_URL; ?>', <?php echo $DNS_Data_R['id']; ?>);">
